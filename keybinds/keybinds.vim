...
function! s:load_clap() abort
  if dein#tap('vim-clap')
    ...
    nnoremap <silent> <Leader>o :<C-u>Clap files ++finder=rg --ignore --hidden --files<cr>
...
let s:plugins = {
  ...
  \ 'sandwich':'vim-sandwich', 'fugitive': 'vim-fugitive',
...
function! s:load_keybinds(plugins) abort
  ...
  while g:config_disable_mappings
    return
  endwhile
  if empty(g:config_disable_pmaping)
    call s:load_plugins_keybinds(pmap)
  else
    for i in g:config_disable_pmaping
